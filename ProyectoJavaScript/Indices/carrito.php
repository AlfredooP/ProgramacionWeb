...
if (isset($_GET['producto']) && isset($_GET['eliminar']) && isset($_GET['cantidad']) && $_GET['eliminar'] == 'false') {
    ...
    if ($producto != 0) {
        $consulta = "INSERT INTO carrito (id, precio, color, talla, marca, modelo, imagen, cantidad) SELECT id, precio, color, talla, marca, modelo, imagen, ".$cantidad." FROM tenis WHERE id = '" . $producto . "';";
        mysqli_query($conexion, $consulta);
    }
